...
class LDLLDM_Full: 
    ...
    def objective_gd(self, manifold):
        @pymanopt.function.numpy(manifold)
        def cost(W):
            l, _ = self.LDL(W.reshape(-1,))
            return l
        
        @pymanopt.function.numpy(manifold)
        def grad(W):
            _, g = self.LDL(W.reshape(-1, 1))
            return g.reshape((self.n_features, -1))
    ...
    def solve_gd(self):
        ...
        cost, grad = self.objective_gd(manifold)
...
class LDLLDM_Incom: 
    ...
    def solve_gd(self, weights, max_iters = 200):
        manifold = Euclidean(self.n_features, self.n_outputs)
    ...
    def solve_BFGS(self, weights, max_iters = 200):
